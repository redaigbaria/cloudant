...
// 		print_r($this->query_parameters);
	...
	public function setQueryParameters(array $options) {
		foreach ($options as $option => $v) {
			if (array_key_exists($option, $this->query_defs)) {
				$this->$option($v);
			}
		}
...
// 			return $this->_continuousChanges();
	...
	/**
	 * Perform a cloudant query
	 * @param  Array $query query paramters
	 * @return stdClass       query result
	 */
	public function find($query) {
		$http = curl_init();
		curl_setopt($http, CURLOPT_URL, $this->getDatabaseUri() . '/_find');
		$query = json_encode($query);
		curl_setopt($http, CURLOPT_POST, sizeof($query));
	 ...
	 * @param  string $ddoc  desgin document id
	 * @param  string $sdoc  search name
	 * @param  array $query query
	 * @return object        result
	 */
	public function search($ddoc = null,$sdoc,$query=null)
	{
		if($query == null || $ddoc == null) {
		...
		curl_setopt($http, CURLOPT_URL, $this->getDatabaseUri() . '/_design/'.$ddoc .'/_search/' . "{$sdoc}?" . $query );
		curl_setopt($http, CURLOPT_HEADER, true);
		curl_setopt($http, CURLOPT_RETURNTRANSFER, true);
		curl_setopt($http, CURLOPT_FOLLOWLOCATION, true);
		$result = curl_exec($http);
